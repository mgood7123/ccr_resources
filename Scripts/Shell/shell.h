...
#include <ctype.h>
#include <assert.h>
#include "builtins/usage.h"
#include "builtins/env.h"
#include "builtins/colors.h"
...
#define CURRENT_FUNCTION printf("->called %s() at line %d from %s\n", __func__, __LINE__, __FILE__);
#endif
...
#define shell_minimal
#include "builtins/argv.h"
...
#define iftimeretc(x, ret) { x; return ret; }
...
// 	DEBUG printf("machTid is %d\n", machTID);
    ...
    if (shell.exe == true && shell.internal == false && shell.builtin == false) {
    	...
    	ps(exe)
    	list_array(cmd->vector_word, "vector");
    	ps(*cmd->vector_word)
    	pi(*(cmd->vector_word))
    	ps(*(cmd->vector_word))
    	pi(*(cmd->vector_word+1))
    	ps(*(cmd->vector_word+1))
    	pi(*(cmd->vector_word+2))
    	ps(*(cmd->vector_word+2))
    	int ret;
    	if(exe) {
    		ret = 0;
    		iftime(pi(execvp(exe, cmd->vector_word)));
    	}
    	else {
    		iftime()
    		ret = -1;
...
// 	DEBUG printf("machTid is %d\n", machTID);
	...
	for (t = 1; t < 2; t++) {
		// to enablee background execution, without the ability to independantly exit, comment out from here
