...
struct QNode* newNode(int one, int two, char * three, bool five, struct Queue *q2)
{
...
void store_asm(struct Queue *q, int one, int two, char * three, bool five, struct Queue *q2)
{
    // Create a new LL node
    struct QNode *temp = newNode(one, two, three, five, q2);
...
int qd_queue_add(struct Queue *q1, int one, int two, char * three, bool five, struct Queue *q2) {
    if (!q1) q1 = createQueue();
    store_asm(q1, one, two, three, five, q2);
    return 0;
}
...
void dis(char * binary) {
	...
	// BEYOND THIS POINT IS GENERATED CODE MANUALLY MODIFIED
	
void code_1(void) {
                printf("a->contents = 0011, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 16 */
                puts("FIN1 ");/* indent level: 16 */
                /* indent level: 8 */
        /*ACTION CODE BEGIN*//* puts($$.name); *//*ACTION CODE END*//* indent level: 8 */
        puts("RP ");/* indent level: 16 */
}
void code_2(void) {
                        printf("a->contents = 000, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
}
void code_3(void) {
...
void code_10(void) {
        /*ACTION CODE BEGIN*//* puts($$.content); *//*ACTION CODE END*//* indent level: 8 */
}
void code_11(void) {
        ...
        /* indent level: 16 */
				...
				struct Queue * qs0 = NULL;
				qd_queue_add(qs0, 3,3,"2-0,000",false, NULL, &code_2); // if
				qd_queue_add(qs0, 3,3,"2-0,001",true, NULL, &code_3); // else if
				qd_queue_add(qs0, 3,3,"2-0,010",true, NULL, &code_4); // else if
				qd_queue_add(qs0, 3,3,"2-0,011",true, NULL, &code_5); // else if
				qd_queue_add(qs0, 3,3,"2-0,100",true, NULL, &code_6); // else if
				qd_queue_add(qs0, 3,3,"2-0,101",true, NULL, &code_7); // else if
				qd_queue_add(qs0, 3,3,"2-0,110",true, NULL, &code_8); // else if
				qd_queue_add(qs0, 3,3,"2-0,111",true, NULL, &code_9); // else if
			struct Queue * qs1 = NULL;
			qd_queue_add(qs1, NULL,NULL,NULL,NULL, qs0, &code_10); // else if
			qd_queue_add(qs1, 1,1,"0,0",false, NULL, &code_11); // if

		struct Queue * q1 = NULL;
		qd_queue_add(q1, 4,4,"3-0,0011",false, qs1, &code_1); // if
		
			struct Queue * qs2 = NULL;
			qd_queue_add(qs2, 3,3,"2-0,000",false, NULL); // if
			qd_queue_add(qs2, 3,3,"2-0,001",true, NULL); // else if
			qd_queue_add(qs2, 3,3,"2-0,010",true, NULL); // else if
			qd_queue_add(qs2, 3,3,"2-0,011",true, NULL); // else if
			qd_queue_add(qs2, 3,3,"2-0,100",true, NULL); // else if
			qd_queue_add(qs2, 3,3,"2-0,101",true, NULL); // else if
			qd_queue_add(qs2, 3,3,"2-0,110",true, NULL); // else if
			qd_queue_add(qs2, 3,3,"2-0,101",true, NULL); // else if
			qd_queue_add(qs2, 3,3,"2-0,101",true, NULL); // else if
			qd_queue_add(qs2, 1,1,"0,1",false, NULL); // if

		struct Queue * q2 = NULL;
		qd_queue_add(q2, 4,4,"3-0,0011",false, qs2); // else if
		
			struct Queue * qs3 = NULL;
			qd_queue_add(qs3, 4,4,"3-0,0xx1",false, NULL); // if
			qd_queue_add(qs3, 4,4,"3-0,1xx1",true, NULL); // else if
			qd_queue_add(qs3, 4,4,"3-0,0x1x",true, NULL); // else if
			qd_queue_add(qs3, 4,4,"3-0,1x1x",true, NULL); // else if
			qd_queue_add(qs3, 4,4,"3-0,01xx",true, NULL); // else if
			qd_queue_add(qs3, 4,4,"3-0,11xx",true, NULL); // else if
			qd_queue_add(qs3, 4,4,"3-0,xxxx",false, NULL); // if
			qd_queue_add(qs3, 4,4,"3-0,xxxx",false, NULL); // if

		struct Queue * q3 = NULL;
		qd_queue_add(q3, 4,4,"3-0,0001",false, qs3); // else if
    ...
    while (node != NULL) {
        ...
        node = load_asm(qd);
        if (node == NULL) break;
		if (!regexEngineb(node->one, node->two, node->three, binstr.data, node->five)) {
			// ...
		}
        nodes++;
    }
    if (nodes == 0) puts("i have no nodes");
    else puts("i have no nodes left");
/* indent level: 8 */
        /*ACTION CODE BEGIN*//* puts($$.name); *//*ACTION CODE END*//* indent level: 8 */
        if (!regexEngineb(4, 4, "3-0,0011", binstr.data, false)) {/* indent level: 16 */
                printf("a->contents = 0011, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 16 */
                puts("FIN1 ");/* indent level: 16 */
                /* indent level: 8 */
        /*ACTION CODE BEGIN*//* puts($$.name); *//*ACTION CODE END*//* indent level: 8 */
        puts("RP ");/* indent level: 16 */
                if (!regexEngineb(3, 3, "2-0,000", binstr.data, false)) {/* indent level: 24 */
                        printf("a->contents = 000, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }else if (!regexEngineb(3, 3, "2-0,001", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 001, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }else if (!regexEngineb(3, 3, "2-0,010", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 010, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }else if (!regexEngineb(3, 3, "2-0,011", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 011, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }else if (!regexEngineb(3, 3, "2-0,100", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 100, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }else if (!regexEngineb(3, 3, "2-0,101", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 101, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }else if (!regexEngineb(3, 3, "2-0,110", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 110, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }else if (!regexEngineb(3, 3, "2-0,111", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 111, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }/* indent level: 8 */
        /*ACTION CODE BEGIN*//* puts($$.content); *//*ACTION CODE END*//* indent level: 8 */
        if (!regexEngineb(1, 1, "0,0", binstr.data, false)) {/* indent level: 24 */
                        printf("a->contents = 0, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 8 */
        /*ACTION CODE BEGIN*//* puts($$.name); *//*ACTION CODE END*//* indent level: 8 */
        /* indent level: 16 */
                } else read__string_undo(binstr.data);/* indent level: 16 */
                /* indent level: 8 */
        }/* indent level: 8 */
        /*ACTION CODE BEGIN*//* puts($$.name); *//*ACTION CODE END*//* indent level: 8 */
         else if (!regexEngineb(4, 4, "3-0,0011", binstr.data, true)) {/* indent level: 16 */
                printf("a->contents = 0011, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 16 */
                puts("FIN2 ");/* indent level: 16 */
                /* indent level: 8 */
        /*ACTION CODE BEGIN*//* puts($$.name); *//*ACTION CODE END*//* indent level: 8 */
        puts("RP ");/* indent level: 16 */
                if (!regexEngineb(3, 3, "2-0,000", binstr.data, false)) {/* indent level: 24 */
        ...
        /*ACTION CODE BEGIN*//* puts($$.content); *//*ACTION CODE END*//* indent level: 8 */
        if (!regexEngineb(1, 1, "0,1", binstr.data, false)) {/* indent level: 24 */
        ...
        /*ACTION CODE BEGIN*//* puts($$.name); *//*ACTION CODE END*//* indent level: 8 */
         else if (!regexEngineb(4, 4, "3-0,0001", binstr.data, true)) {/* indent level: 16 */
                printf("a->contents = 0001, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 16 */
                puts("JCN ");/* indent level: 16 */
                puts("CN ");/* indent level: 16 */
                if (!regexEngineb(4, 4, "3-0,0xx1", binstr.data, false)) {/* indent level: 24 */
                        printf("a->contents = 0xx1, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }if (!regexEngineb(4, 4, "3-0,1xx1", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 1xx1, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }if (!regexEngineb(4, 4, "3-0,0x1x", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 0x1x, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }if (!regexEngineb(4, 4, "3-0,1x1x", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 1x1x, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }if (!regexEngineb(4, 4, "3-0,01xx", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 01xx, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }if (!regexEngineb(4, 4, "3-0,11xx", binstr.data, true)) {/* indent level: 24 */
                        printf("a->contents = 11xx, regexEngineb_bits = %s\n", regexEngineb_bits);/* indent level: 24 */
                        /* indent level: 16 */
                }puts("ADDR ");/* indent level: 16 */
                if (!regexEngineb(4, 4, "3-0,xxxx", binstr.data, false)) {/* indent level: 24 */
