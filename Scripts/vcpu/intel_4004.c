...
struct CPUQNode* CPUnewNode(char * string);
struct CPUQueue *CPUcreateQueue();
void CPUstore_asm(struct CPUQueue *q, char * string);
struct CPUQNode * CPUload_asm(struct CPUQueue *q);
...
struct CPUQueue
{
    struct CPUQNode *front, *rear;
};
 
// A utility function to create a new linked list node.
struct CPUQNode* CPUnewNode(char * string)
{
    struct CPUQNode *temp = (struct CPUQNode*)malloc(sizeof(struct CPUQNode));
...
struct CPUQueue *CPUcreateQueue()
{
    struct CPUQueue *q = (struct CPUQueue*)malloc(sizeof(struct CPUQueue));
    q->front = q->rear = NULL;
    return q;
}
...
void CPUstore_asm(struct CPUQueue *q, char * string)
{
    ...
    struct CPUQNode *temp = CPUnewNode(string);
...
int cpu_queue_add(char * string) {
    if (!cpu_default.q) cpu_default.q = CPUcreateQueue();
    CPUstore_asm(cpu_default.q, string);
    return 0;
}
...
void cpu_execute(struct virtual_cpu *cpu) {
    ...
    struct CPUQNode * node = malloc(1); // this gets freed anyway
    int instructions = 0;
    while (node != NULL) {
        // drain the list until empty
        free(node);
        node = CPUload_asm(cpu->q);
        if (node == NULL) break;
